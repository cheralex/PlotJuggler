...
bool DataLoadAPBIN::readDataFromFile(FileLoadInfo* info, PlotDataMapRef& plot_data)
{
  ...
  while (true)
  {
    ...
    handle_message_received(format, &buf[total_bytes_used], plot_data);
...
void DataLoadAPBIN::handle_message_received(const struct log_Format& format, const uint8_t* msg, PlotDataMapRef& plot_data)
{
  ...
  {
    PlotData& series = plot_data.getOrCreateNumeric("/" + fname + "/" + timeseries.data[i].first);

    timesec = static_cast<double>(msg_time) * 0.000001;
    ...
    switch (typeCode)
    {
      case 'b':
        ...
        break;
      case 'c':
        value = static_cast<double>(*reinterpret_cast<const int16_t*>(msg + msg_offset)) / 100.;
        msg_offset += sizeof(int16_t);
        break;
      case 'e':
        value = static_cast<double>(*reinterpret_cast<const int32_t*>(msg + msg_offset)) / 100.;
      ...
      case 'B':
        ...
        break;
      case 'C':
        value = static_cast<double>(*reinterpret_cast<const uint16_t*>(msg + msg_offset)) / 100.;
        msg_offset += sizeof(uint16_t);
        break;
      case 'E':
        value = static_cast<double>(*reinterpret_cast<const uint32_t*>(msg + msg_offset)) / 100.;
    ...
    if (timeseries.data[i].first.compare("TimeUS") == 0) {
        timesec = value * 0.0000001;
    } if (timeseries.data[i].first.compare("TimeMS") == 0) {
        timesec = value * 0.001;
    }

    series.pushBack(PlotData::Point(timesec, value));
  }
}
